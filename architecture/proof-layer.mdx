...
<div className="usb-section">
  <h2>Why it exists</h2>
  <p>
    Automation only works if every participant can trust the results. USB.Trade combines Risc0’s Boundless Network for zero-knowledge proofs with MaldaLab’s sequencer technology to validate that automation modules followed policy, touched approved venues, and settled in the right order.
  </p>
  <p>
    Users stay in custody while gaining cryptographic assurance that orchestration decisions were executed faithfully—even when they span multiple chains.
  </p>
</div>
...
<div className="usb-section">
  ...
  <div className="usb-grid">
    <div className="usb-card">
      ...
      <p className="usb-card__body">Automation actions are logged with signatures, calldata, and oracle inputs so they can be fed into Risc0 circuits.</p>
    </div>
    <div className="usb-card">
